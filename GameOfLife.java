...
public class GameOfLife {
	...
	public static int[][] read(String fileName) {
		...
		int[][] board = new int[rows + 2][cols + 2]; //array that receives ints
		String rowString = "";

		for (int i = 1; i <= rows; i++) { //reads each row
			rowString = in.readLine(); //reads string in each row and saves in rowString
			for (int j = 1; j < rowString.length() + 1; j++) { //each char in row for 
	...
	public static int[][] evolve(int[][] board) {
		...
		int[][] newboard = new int[rows][cols]; //duplicates board to new board so it doesn't replace old one
		for (int i = 1; i < rows - 2; i++){ //for the middle rows in the board
			for (int j = 1; j < cols - 2; j++){ //for the middle columns in the board
	...
	public static int count(int[][] board, int i, int j) {
		...
		if (board[i - 1][j] == 1) //top
			count++;
		if (board[i - 1][j + 1] == 1) //bottom left corner
			count++;
		...
		if (board[i + 1][j] == 1) //bottom
			count++;
		if (board[i + 1][j + 1] == 1) //bottom right corner
			count++;
		return count;
    ...
    public static void print(int[][] arr) {
		int N = arr.length - 1; //number of rows in board
		int M = arr[0].length - 1; //number of columns in board
